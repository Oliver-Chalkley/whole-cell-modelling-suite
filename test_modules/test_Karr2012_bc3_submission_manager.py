...
class Karr2012Bc3ManagerTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_createCommandToConvertMatFilesToPicklesUT(self):
        ...
        if self.test_all_state_conversions == True:
            actual_cmd = ['module add languages/python-anaconda-4.2-3.5', 'source activate wholecell_modelling_suite', 'python -c "import sys;sys.path.insert(0, \'/panfs/panasas01/bluegem-flex1/database/functions_for_extracting_data_from_matFiles/functions_for_joshuas_matFiles\');import extract_matFile_data_v73 as extract;list_or_dict_of_simdir_and_save_dir = /panfs/panasas01/bluegem-flex1/database/wcm_suite/unittest/wcms/tmp_sim;extract.saveBasicDetailsAsPickle(list_or_dict_of_simdir_and_save_dir);extract.saveMatureRnaCountsAsPickle(list_or_dict_of_simdir_and_save_dir);extract.saveMetabolicReactionFluxsAsPickle(list_or_dict_of_simdir_and_save_dir);extract.saveMatureProteinMonCountsAsPickle(list_or_dict_of_simdir_and_save_dir);extract.saveMatureProteinComCountsAsPickle(list_or_dict_of_simdir_and_save_dir)"']
        else:
            actual_cmd = ['module add languages/python-anaconda-4.2-3.5', 'source activate wholecell_modelling_suite', 'python -c "import sys;sys.path.insert(0, \'/panfs/panasas01/bluegem-flex1/database/functions_for_extracting_data_from_matFiles/functions_for_joshuas_matFiles\');import extract_matFile_data_v73 as extract;list_or_dict_of_simdir_and_save_dir = /panfs/panasas01/bluegem-flex1/database/wcm_suite/unittest/wcms/tmp_sim;extract.saveBasicDetailsAsPickle(list_or_dict_of_simdir_and_save_dir)"']
        ...
        # create the command
        cmd_to_test = self.test_manager.createCommandToConvertMatFilesToPickles('/panfs/panasas01/bluegem-flex1/database/wcm_suite/unittest/wcms/tmp_sim')
    ...
    def test_convertMatToPandasUT(self):
        ...
        raw_out = self.bc3_conn.checkSuccess(self.bc3_conn.sendCommand, copy_cmd)
        if raw_out['return_code'] != 0:
            raise ValueError("Whilst constructing the class we couldn't create remote directories!. raw_out = ", raw_out)
    ...
    def test_updateDbGenomeReduction2017(self):
        ...
        pathlib.Path(self.bc3_unittest_ko_sub.temp_storage_path).mkdir(parents=True, exist_ok=True)
        outs =  self.bc3_unittest_ko_sub.prepareForSubmission()
        ...
        raw_out1 = self.bc3_conn.checkSuccess(self.bc3_conn.sendCommand, test_db_cmd)
        raw_out1_test = {'return_code': 0, 'stdout': '', 'stderr': None}
        self.test_manager.updateDbGenomeReduction2017()
        raw_out2 = self.bc3_conn.checkSuccess(self.bc3_conn.sendCommand, test_db_cmd)
