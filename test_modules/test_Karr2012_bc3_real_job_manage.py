...
class Karr2012Bc3RealManageTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        cls.master_dir = '/panfs/panasas01/bluegem-flex1/database/WholeCell-master'
        ...
        cls.queue_nameUT = 'veryshort'

        # CREATE SUBIMSSION INSTANCES
        cls.bc3_wcm_ko_sub = job_management.SubmissionKarr2012(cls.submission_name, cls.bc3_conn, cls.ko_name_to_set_dict, cls.queue_name, cls.full_output_path, cls.full_runfiles_path, cls.full_out_and_error_files, cls.full_out_and_error_files, cls.master_dir, cls.repetitions_of_a_unique_simulation, cls.createAllFilesFunctionName, cls.createDataDictForSpecialistFunctionsFunctionName, cls.createSubmissionScriptFunctionName, cls.createDictOfFileSourceToFileDestinationsFunctionName, first_wait_time = 15, second_wait_time = 15, temp_storage_path = cls.temp_storage_path)

        # CREATE A UNITTEST SUBMISION INSTANCE
        cls.master_dir = '/panfs/panasas01/bluegem-flex1/database/wcm_suite/unittest/unittest-master'
        ...
        cls.bc3_unittest_ko_sub = job_management.SubmissionKarr2012(cls.submission_name, cls.bc3_conn, cls.ko_name_to_set_dict, cls.queue_nameUT, cls.full_output_path, cls.full_runfiles_path, cls.full_out_and_error_files, cls.full_out_and_error_files, cls.master_dir, cls.repetitions_of_a_unique_simulation, cls.createAllFilesFunctionNameUT, cls.createDataDictForSpecialistFunctionsFunctionNameUT, cls.createSubmissionScriptFunctionNameUT, cls.createDictOfFileSourceToFileDestinationsFunctionNameUT, first_wait_time = 1, second_wait_time = 1, temp_storage_path = cls.temp_storage_path)

        # CREATE A JOB MANAGEMENT INSTANCE IN TEST MODE
        #cls.test_manager = job_management.SubmissionManagerKarr2012(cls.bc3_unittest_ko_sub, cls.convertDataFunctionName, cls.updateCentralDbFunctionName, cls.getDataForDbFunctionName, list_of_states_to_save = list_of_states_to_convert, ko_db_path_relative_to_db_connection_flex1 = 'database/ko_db/unittest_ko_db/unittest_ko.db', test_mode = True)
        ...
        source_path = '/panfs/panasas01/bluegem-flex1/database/mat_files/version73_example_mats_files/1/' # put the trailing slash so that it only copies the contents of the dir
        transfer_cmd = ['rsync -aP ' + source_path + ' ' + cls.full_path_to_sim_example]
        raw_out = cls.bc3_conn.checkSuccess(cls.bc3_conn.sendCommand, transfer_cmd)
        if raw_out['return_code'] != 0:
    ...
    def test_monitorSubmission(self):
        ...
        clear_db_cmd = ['cd /panfs/panasas01/bluegem-flex1/database/ko_db/unittest_ko_db; rm unittest_ko.db; sqlite3 unittest_ko.db < unittest_ko_db.schema']
        raw_out = self.bc3_conn.checkSuccess(self.bc3_conn.sendCommand, clear_db_cmd)
        if raw_out['return_code'] != 0:
            raise ValueError('Could not create a fresh unittest database". raw_out = ', raw_out)
        
        gene_code_to_id_dict = self.getDictOfJr358Codes(self.bc3_conn)
        test_manager = job_management.SubmissionManagerKarr2012(self.bc3_wcm_ko_sub, gene_code_to_id_dict, self.convertDataFunctionName, self.updateCentralDbFunctionName, self.getDataForDbFunctionName, ko_db_path_relative_to_db_connection_flex1 = 'database/ko_db/unittest_ko_db/unittest_ko.db', test_mode = False)
        test_db_cmd = ['sqlite3 /panfs/panasas01/bluegem-flex1/database/ko_db/unittest_ko_db/unittest_ko.db "select count(simulationDetails.average_growth_rate) from simulationDetails join koIndex on simulationDetails.ko_index_id = koIndex.id join batchDescription on simulationDetails.batch_description_id = batchDescription.id join people on people.id = batchDescription.simulation_initiator_id"']
        raw_out = self.bc3_conn.checkSuccess(self.bc3_conn.sendCommand, test_db_cmd)
